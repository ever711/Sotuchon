...
namespace XoSoTuChonBenTre
{
    public partial class Confirm : System.Web.UI.Page
    {
        ...
        protected void btnTim_Click(object sender, EventArgs e)
        {
            ...
            ng1 = (ngay1 - ngay).Days;
            ng2 = (ngay2 - ngay).Days;
            var table = context.Games.Include("TaiKhoanHeThong").Where(p => p.NgayChoi >= ng1 && p.NgayChoi <= ng2 && p.TaiKhoanHeThong != null && p.TaiKhoanHeThong.IDDaiLy == currentUser.IDTaiKhoan)
                .OrderByDescending(x => x.ThoiGianDat);
            var data = table.ToList();
            data.ForEach(x => x.TenTaiKhoan = x.TaiKhoanHeThong.TaiKhoan);
                ...
                using (SoXoTuChon_BenTreEntities context1 = new SoXoTuChon_BenTreEntities())
                {
                    ...
                    foreach (var item in tableGame)
                    {
                        using (SoXoTuChon_BenTreEntities context2 = new SoXoTuChon_BenTreEntities())
                        {
                            ...
                            table3.IDTaiKhoanNap = currentUser.IDTaiKhoan;
        ...
        private string getNguoiXacNhan()
        {
            return Session["tk"] != null && !string.IsNullOrEmpty(Session["tk"].ToString()) ?
                Session["tk"].ToString()
                : Request.Cookies["taikhoan"].Value;
        }
    }
}